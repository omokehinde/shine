...
State.create!(name: "Califonia", code: "CA")
...
State.create!(name: "Geogia", code: "GA")
State.create!(name: "Hawaii", code: "HI")
State.create!(name: "Idaho", code: "ID")
State.create!(name: "Illinios", code: "IL")
...
State.create!(name: "Masschuesetts", code: "MA")
State.create!(name: "Michigan", code: "MI")
State.create!(name: "Minnesota", code: "MN")
State.create!(name: "Mississipi", code: "MS")
...
# Helper method to create billing address for a customer
...
# Cache the number of states so we dont have to query each time through
...
Customer.all.pluck(:id).each do |customer_id|
	...
	# creates a billing address for them
	create_billing_address(customer_id, num_states)

	# creates a random number of shipping addresses, making sure we create at least 1
	num_shipping_addresses = rand(4) + 1

	num_shipping_addresses.times do |i|
		# Creates the shipping addresses, setting the first one we create as the "primary"
